# Project Profile Discovery Directive v1.2

**Version:** v1.2
**Directive date:** 2026-05-08
**Status:** Operator-to-agent instruction document
**Scope:** Project-agnostic and tooling-agnostic
**Lineage:** Supersedes Project Profile Discovery Directive v1.1. Preserves the dated-snapshot model while adding protocol-surface discovery, authority-matrix baselines, execution-mode discovery, sharper state/provenance classes, semantic-convention stability pinning, and companion-artifact drift inputs for Audit Spec v3.1.
...
2. **Evidence-first.** Every output field must cite source files and line ranges, be marked `source: operator-interview`, or be marked `source: unknown` with a reason.
...
Record version pins when manifests expose them.
...
- directive-set companion files (`agentic-audit-spec-v3.md`, `audit-kickoff-prompt.md`, `audit-spec-friendly-explainer.md`, manifests, examples) when this directory is itself an audit-package workspace.
...
3. Validate evidence paths and line ranges where applicable.
...
If the operator supplies a theme name without a custom weighting, the audit applies the default mapping from §11.12 of `agentic-audit-spec-v3.1.md`. The profile records `weighting.dimensions: []` and `weighting.multiplier: 1.5` to indicate "use default mapping for this theme name."
...
This directive is **v1.2** dated 2026-05-08.
...
2. Run v1.2 as a new dated snapshot; do not mutate the prior profile in place.

...
**Derived guidance targets:** Agentic Architecture Audit Specification v3.1 (2026-05-08) and Project Profile Discovery Directive v1.2 (2026-05-08). If this prompt conflicts with the full spec, the full spec wins.

```
You are running the Agentic Architecture Audit per the v3.1 specification at
`agentic-audit-spec-v3.md` (full spec, ~1900 lines). Read that file in full
before doing anything else. This message is the kickoff; the spec is authoritative.
If `audit-directive-set-manifest.md` exists beside the spec, read it after the
...
Run `project-profile-directive.md` using Project Profile Discovery Directive
v1.2 first, then re-invoke this audit."
...
1. Read `agentic-audit-spec-v3.md` in full.
2. Read `audit-directive-set-manifest.md` if present.
